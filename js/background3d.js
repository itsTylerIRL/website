...
function initBackground3D() {
    ...
    renderer.domElement.id = 'bg3d-canvas'; // Targetable by CSS (blackout on manual pause)
...
function animate() {
    ...
    // Paused while tab is hidden (see visibilitychange handler at end of file)
    // or manually via the footer slider tap toggle (window._bgManualPause).
    // rAF is already throttled in background tabs; this makes the pause explicit
    // and skips all per-frame work/rendering.
    if (window._bgPaused || window._bgManualPause) return;
...
// ----- Tab visibility: pause background + CONN LOST / CONNECTED title -----
(function() {
    ...
    document.addEventListener('visibilitychange', () => {
        if (document.hidden) {
            // Pause all per-frame background work
            window._bgPaused = true;
            if (restoreTimer) { clearTimeout(restoreTimer); restoreTimer = null; }
            document.title = 'T: \u26a0 CONN. LOST';
        } else {
            // Resume: reset frame clock so there's no dt jump
            window._bgPaused = false;
            if (typeof animate === 'function') animate._lastTime = null;
            document.title = 'T: \u25cf CONNECTED';
